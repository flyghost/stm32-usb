                                    ...
                                    void *phost)
{
    ...
    switch (CDC_ReqState)
    {
        ...
        case CDC_GET_LINE_CODING_RQUEST:
            ...
            if ( ClassReqStatus == USBH_OK || ClassReqStatus == USBH_NOT_SUPPORTED )  //USBH_NOT_SUPPORTED is STALL 
            {
                /*Change the state */
...
/**
  ...
  * @retval None
  */
void CDC_ProcessReception(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)
{
    if (RX_Enabled == 1)
    {
