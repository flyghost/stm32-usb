...
extern void USBH_CH340_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost);
/**
...
int main(void)
{
    ...
    STM_EVAL_PBInit(Button_KEY, Mode_EXTI);     //K1发送数据
    STM_EVAL_PBInit(Button_TAMPER, Mode_EXTI);  //K2打开串口
    ...
    {
        /* Host Task handler */
        USBH_CH340_Process(&USB_OTG_Core, &USB_Host);
    }
}
